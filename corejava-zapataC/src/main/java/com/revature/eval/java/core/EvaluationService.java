...
public class EvaluationService {
	...
	public String acronym(String phrase) {
		...
		char[] findAcronym = new char[len];

		// put string into an array of characters
		for (int i = 0; i < len; i++) {
			tempCharArray[i] = phrase.charAt(i);
		}

		// is creating empty characters in array
		for (int i = 0, j = 0; i < len; i++) {
			if (i == 0) {
				findAcronym[j] = tempCharArray[i];
				j++;
			} else if (tempCharArray[i] == ' ' || tempCharArray[i] == '-') {
				findAcronym[j] = tempCharArray[++i];
				findAcronym[j] = Character.toUpperCase(findAcronym[j]);
				j++;
			}
		}

		/////////////////////////////////////////////////////////////////////////
		// Create a new array of exact number of characters in the acronym
		int lenAcronym = 0;

		for (int i = 0; i < len; i++) {
			if (Character.isLetter(findAcronym[i])) {
				lenAcronym++;
			}
		}

		char[] perfectArraySize = new char[lenAcronym];

		for (int i = 0; i < lenAcronym; i++) {
			perfectArraySize[i] = findAcronym[i];
		}
		////////////////////////////////////////////////////////////////////////////////

		// Convert array back to string
		String acronym = new String(perfectArraySize);
	...
	public int getScrabbleScore(String string) {
		...
		char[] fivePointer = { 'K', 'k' };
		char[] eightPointers = { 'J', 'X' };
		char[] tenPointers = { 'Q', 'Z' };
		Boolean exists = false;
		int[] pointArray = new int[len];

		// put the string into an array of char and capitalize letter
		for (int i = 0; i < len; i++) {
			charArrEquivalent[i] = string.charAt(i);
			charArrEquivalent[i] = Character.toUpperCase(charArrEquivalent[i]);
		}

		// with the first letter in the array, compare it to a letter in the one pointer
		// array.
		// if a match is found, then add points.
		for (int i = 0; i < len; i++) {
			java.util.Arrays.sort(onePointers);
			java.util.Arrays.sort(twoPointers);
			java.util.Arrays.sort(threePointers);
			java.util.Arrays.sort(fourPointers);
			java.util.Arrays.sort(fivePointer);
			java.util.Arrays.sort(eightPointers);
			java.util.Arrays.sort(tenPointers);
			if ((java.util.Arrays.binarySearch(onePointers, charArrEquivalent[i]) >= 0) && (exists == false)) {
				exists = true;
				pointArray[i] = 1;
			} else if ((java.util.Arrays.binarySearch(twoPointers, charArrEquivalent[i]) >= 0) && (exists == false)) {
				exists = true;
				pointArray[i] = 2;
			} else if ((java.util.Arrays.binarySearch(threePointers, charArrEquivalent[i]) >= 0) && (exists == false)) {
				exists = true;
				pointArray[i] = 3;
			} else if ((java.util.Arrays.binarySearch(fourPointers, charArrEquivalent[i]) >= 0) && (exists == false)) {
				exists = true;
				pointArray[i] = 4;
			} else if ((java.util.Arrays.binarySearch(fivePointer, charArrEquivalent[i]) >= 0) && (exists == false)) {
				exists = true;
				pointArray[i] = 5;
			} else if ((java.util.Arrays.binarySearch(eightPointers, charArrEquivalent[i]) >= 0) && (exists == false)) {
				exists = true;
				pointArray[i] = 8;
			} else if ((java.util.Arrays.binarySearch(tenPointers, charArrEquivalent[i]) >= 0) && (exists == false)) {
				exists = true;
				pointArray[i] = 10;
			} else {
				System.out.println("point value for letter not found");
			}
			exists = false;
		}
		
		// add up sum of points
		for (int i = 0; i < len; i++) {
			points += pointArray[i];
		}
...
//		String word = new String(charArrEquivalent);
